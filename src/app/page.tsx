      ...
      <main>
        ...
        <section className="relative w-full min-h-screen md:h-screen overflow-hidden">
          {heroSlides.map((slide, idx) => (
            ...
            >
              <div className={`bg-gradient-to-r ${slide.color} w-full h-full flex flex-col md:flex-row items-center justify-center md:justify-between px-4 sm:px-8 md:px-16 py-12 md:py-0 gap-6 md:gap-0`}>
                {/* Left Content */}
                <div className="flex-1 z-10 text-white w-full md:w-auto text-center md:text-left">
                  <div className="inline-block mb-3 sm:mb-4 px-3 sm:px-4 py-2 bg-white/20 backdrop-blur-sm border border-white/30 rounded-full">
                    <span className="text-xs sm:text-sm font-semibold">{slide.subtitle}</span>
                  </div>
                  ...
                  <div className="flex gap-3 sm:gap-4 flex-col sm:flex-row sm:flex-wrap justify-center md:justify-start">
                    ...
                    <button className="px-6 sm:px-8 py-3 sm:py-4 border-2 border-white text-white font-bold rounded-lg hover:bg-white/10 transition text-sm sm:text-base">
                      Learn More
                    </button>
                  </div>
                </div>
            ...
            {heroSlides.map((_, idx) => (
              <button
                ...
                className={`h-2 sm:h-3 rounded-full transition-all duration-300 ${
                  idx === currentSlide ? 'w-8 sm:w-12 bg-white' : 'w-2 sm:w-3 bg-white/50 hover:bg-white/75'
                }`}
              />
            ))}
          </div>
        ...
        <section className="py-20 px-4 bg-white">
          <div className="max-w-7xl mx-auto">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
              {[
                { icon: '✨', title: 'Premium Quality', desc: 'Crafted with the finest materials and attention to detail' },
                { icon: '🚚', title: 'Fast Shipping', desc: 'Quick and reliable delivery to your doorstep' },
                { icon: '💝', title: 'Customer Care', desc: 'Dedicated support for your satisfaction' }
              ].map((feature, idx) => (
                <div key={idx} className="text-center p-8 rounded-lg hover:shadow-lg transition">
        ...
        <section className="bg-gray-50 py-20 px-4">
          <div className="max-w-7xl mx-auto">
            <div className="text-center mb-16">
              ...
              <p className="text-gray-600 text-lg">Discover our premium shoe collections</p>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              {[
                { title: 'Running Shoes', desc: 'Performance meets comfort', color: 'from-blue-500 to-blue-600' },
                { title: 'Casual Wear', desc: 'Everyday elegance', color: 'from-purple-500 to-purple-600' },
                { title: 'Formal Shoes', desc: 'Sophisticated style', color: 'from-gray-700 to-gray-800' },
                { title: 'Sports Footwear', desc: 'Built for athletes', color: 'from-red-500 to-red-600' },
              ].map((collection, idx) => (
                <Link key={idx} href={`/products?category=${collection.title.toLowerCase().replace(' ', '-')}`} className="group">
                  <div className={`bg-gradient-to-br ${collection.color} h-72 rounded-lg overflow-hidden relative flex items-end justify-start p-8 text-white hover:shadow-2xl transition transform hover:scale-105`}>
                    <div className="relative z-10">
                      <h3 className="text-4xl font-black mb-2">{collection.title}</h3>
                      <p className="text-lg opacity-90 mb-4">{collection.desc}</p>
        ...
        <section className="bg-white py-20 px-4">
          <div className="max-w-7xl mx-auto">
            <div className="text-center mb-16">
              <span className="text-yellow-500 font-bold text-lg">EXPLORE OUR RANGE</span>
              <h2 className="text-5xl font-black mt-2 mb-4">Featured Collections</h2>
            </div>
            <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
              {['Running', 'Casual', 'Formal', 'Sports', 'Premium'].map((cat, i) => (
                <Link key={i} href={`/products?category=${cat.toLowerCase()}`}>
                  <div className="bg-gray-50 p-6 rounded-lg text-center hover:shadow-lg transition hover:bg-yellow-50 cursor-pointer">
                    ...
                    <p className="font-bold text-sm md:text-base">{cat}</p>
                  </div>
                </Link>
              ))}
            </div>
        ...
        <section className="py-20 px-4 bg-gray-50">
          <div className="max-w-7xl mx-auto">
            <div className="text-center mb-16">
              <span className="text-yellow-500 font-bold text-lg">OUR BEST SELLING ITEMS</span>
              <h2 className="text-5xl font-black mt-2 mb-4">Customer Favorites</h2>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
              {[1, 2, 3, 4].map((product) => (
                <div key={product} className="group bg-white rounded-lg overflow-hidden hover:shadow-2xl transition">
                  ...
                  <div className="p-4">
                    <h3 className="font-bold text-lg mb-2">Premium Shoe {product}</h3>
                    <p className="text-gray-600 text-sm mb-4">High-quality footwear</p>
                    <div className="flex items-center gap-2 mb-4">
                      <span className="text-2xl font-black text-yellow-500">$59</span>
                      <span className="text-lg line-through text-gray-400">$89</span>
                    </div>
        ...
        <section className="py-20 px-4 bg-white">
          <div className="max-w-7xl mx-auto">
            <div className="text-center mb-16">
              <span className="text-yellow-500 font-bold text-lg">WHAT PEOPLE ARE SAYING</span>
              <h2 className="text-5xl font-black mt-2 mb-4">Customer Reviews</h2>
              ...
              {[1, 2, 3].map((review) => (
                <div key={review} className="bg-gray-50 p-8 rounded-lg">
                  ...
                  <p className="text-gray-700 mb-4">
                    "Absolutely love these shoes! The quality is exceptional and they're so comfortable. Highly recommended!"
                  </p>
        ...
        <section className="py-20 px-4 bg-gradient-to-r from-yellow-400 to-yellow-500">
          <div className="max-w-4xl mx-auto text-center">
            <h2 className="text-5xl font-black mb-4 text-black">Ready to Step Into Style?</h2>
            <p className="text-xl text-black/80 mb-8">Join thousands of satisfied customers and discover your perfect shoe</p>
        ...
        <section className="bg-white py-16 px-4 border-t-2 border-yellow-400">
          <div className="max-w-2xl mx-auto text-center">
            ...
            <p className="text-gray-600 mb-8">Subscribe to our newsletter and get 10% off your first purchase</p>
        ...
        <div className="max-w-7xl mx-auto">
          <div className="grid grid-cols-1 md:grid-cols-4 gap-8 mb-8">
            <div>
              ...
              <p className="text-gray-400">Premium footwear for those who dare to stand out. Quality, style, and comfort in every step.</p>
            </div>
            <div>
              ...
              <ul className="space-y-2 text-gray-400">
                ...
                <li><Link href="/products?category=running" className="hover:text-yellow-400">Running</Link></li>
                <li><Link href="/products?category=casual" className="hover:text-yellow-400">Casual</Link></li>
                <li><Link href="/products?category=formal" className="hover:text-yellow-400">Formal</Link></li>
              </ul>
            </div>
            <div>
            ...
            <div>
              ...
              <p className="text-gray-400 mb-2">Email: support@classychain.com</p>
          ...
          <div className="border-t border-gray-700 pt-8 text-center text-gray-400">
            <p>&copy; 2026 ClassyChain. All rights reserved. Premium Footwear Store.</p>
          </div>
        </div>
      </footer>
    </>
